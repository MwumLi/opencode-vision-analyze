...
- [x] 写 `docs/superpowers/specs/2026-09-08-description-cache-persist-design.md`
- [x] 写 `docs/superpowers/plans/2026-09-08-description-cache-persist.md`
- [x] 切分支 `feat/description-cache-persist`
- [x] commit `docs: spec & plan for persistent description cache (sha-addressed, LRU/capped)`

## Task 2 · 测试先行（红）：新增用例

- [x] `test/plugin.test.ts` 新增（先红）：
...
- [x] 门禁：全绿（新用例绿 + 既有零 diff）
- commit `feat: persist description cache to user cache dir with LRU cap`

## Task 4 · README + 头部注释收尾

- [x] `README.md` / `README.zh.md`：特性段、原理图、已知限制、Roadmap 勾选
- [x] `src/index.ts` 头部注释同步
- [x] 门禁：typecheck / test / build 全绿
- commit `docs: document persistent description cache (LRU/capped, user-level)`

## Task 5 · 终验与交付

- [x] `bun run typecheck && bun test && bun run build` 全绿
